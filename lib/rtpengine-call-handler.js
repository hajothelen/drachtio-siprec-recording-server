...
const {getAvailableRtpengine, parseSipUserToUser} = require('./utils');
...
module.exports = (req, res) => {
  ...
  const u2u = parseSipUserToUser(req.get('User-to-User'));
  const totag = v4();
...
function allocateEndpoint(which, rtpEngine, totag, opts) {
  ...
  opts.logger.info(`caller: ${opts.caller}`);
  opts.logger.info(`callee: ${opts.callee}`);
  opts.logger.info(`ucid: ${opts.ucid}`);
