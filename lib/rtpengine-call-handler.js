...
const wrad = require('./wrad/wradutils');
...
module.exports = (req, res) => {
  ...
  const u2u = wrad.parseSipUserToUser(req.get('User-to-User'));
  const totag = v4();
  ...
  parseSiprecPayload(opts)
    ...
    .then((dlg) => {
      ...
      dlg.on('modify', onCallModify.bind(null, rtpEngine, logger, totag));
...
function onCallModify(rtpEngine, logger, totag, req, res) {
  logger.info("MOD MOD");
  logger.info("MOD mdified");
  logger.info("MOD MOD");
  const callid = req.get('Call-ID');
  const from = req.getParsedHeader('From');
  const opts = {
...
function onCallEnd(rtpEngine, opts) {
  ...
  return rtpEngine.delete(rtpEngine.remote, opts.callDetails)
    .then((response) => {
      opts.logger.info(`END END`)
      opts.logger.info(`END Response: ${JSON.stringify(response)}`)
      opts.logger.info(`END END`)
...
function allocateEndpoint(which, rtpEngine, totag, opts) {
  ...
  opts.logger.info(`sipheaders: ${opts.sipheaders}`);
  opts.logger.info(`caller: ${JSON.stringify(opts.caller)}`);
  opts.logger.info(`callee: ${JSON.stringify(opts.callee)}`);
  opts.logger.info(`ucid: ${opts.u2u.ucid}`);
  ...
  return rtpEngine[which === 'caller' ? 'offer' : 'answer'](rtpEngine.remote, args)
    .then((response) => {
      opts.logger.info(`XXXXXXXXXXX`)
      opts.logger.info(`XX Response: ${JSON.stringify(response)}`)
      opts.logger.info(`XXXXXXXXXXX`)
...
/**
 * Create a Dialog as user agent server
 * @param {*} opts 
 * @returns an promise with a srf.Dialog
 */
